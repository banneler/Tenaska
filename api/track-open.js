...
function buildEmail({ openedAt, ip, location, path, referrer, userAgent }) {
  const mapsLink =
  ...
  const html = `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#0f172a;line-height:1.5;">
      <h2 style="margin:0 0 12px;font-size:20px;">Tenaska follow-up opened</h2>
      <p style="margin:0 0 18px;color:#475569;">Someone loaded the follow-up presentation link.</p>
      <table style="border-collapse:collapse;width:100%;max-width:560px;">
        <tr><td style="padding:8px 0;color:#64748b;width:160px;">Opened at</td><td style="padding:8px 0;font-weight:600;">${escapeHtml(openedAt)}</td></tr>
...
module.exports = async function handler(req, res) {
  ...
  if (!GMAIL_APP_PASSWORD) {
    return res.status(500).json({ ok: false, error: 'Email sender is not configured' });
  }
  ...
  try {
    ...
    const path = String(body.path || '/follow-up/').slice(0, 300);
    ...
    const { text, html } = buildEmail({ openedAt, ip, location, path, referrer, userAgent });
    ...
    await transporter.sendMail({
      from: `"Tenaska Mapbook" <${FROM_EMAIL}>`,
      to: TO_EMAIL,
      subject: `Tenaska follow-up opened — ${location.summary}`,
      text,
      html
    });
